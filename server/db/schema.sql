-- ============================================================================
-- Map of Knowledge — base database schema (structure only, no data)
...
--
-- After this runs, migrate.js also loads app/knowledge_map*.json into the
-- now-empty nodes/edges tables (its existing "if nodes count > 0, skip
-- import" logic already handles that correctly on a fresh DB).
--
-- To regenerate this file after a schema change: run the same mysqldump
-- command above against production and replace everything below this header
-- (see docs/operations.md's Database section for the exact command with
-- credentials — that file is local-only/gitignored, this one is not).
-- ============================================================================
