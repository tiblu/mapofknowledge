...
-- Target  : MariaDB 11.4 / MySQL (zone.ee, host d148395.mysql.zonevs.eu)
-- Snapshot of the LIVE production DB as of 2026-09-01 (regenerated via
-- `mysqldump --no-data --routines --triggers` against d148395sd640285 — this
-- is what is actually deployed, not an aspirational design doc). Supersedes
-- docs/map_of_knowledge_schema_V2.sql (2026-06-17, 29 tables) — that file is
-- kept for historical reference, not deleted.
--
-- 39 tables across six concerns:
--   1. Users & authentication       (users, oauth_identities, user_settings,
--                                    notifications, webauthn_credentials)
--   2. Learner passport             (learner_passports + 11 passport_* tables,
--                                    friend_invites, anne_messages,
--                                    learner_whois)
--   3. Knowledge map & learning     (nodes, node_translations, edges, knobits,
--                                    knobit_progress, knobit_interactions,
--                                    user_node_knowledge, llm_usage_log,
--                                    token_usage, knowledge_estimate_log,
--                                    lootbox_cache)
--   4. Gamification                 (lumen_transactions, user_achievements,
--                                    user_momentum, user_streaks)
--   5. Knowledge subsets / filters  (knowledge_subsets, knowledge_subset_nodes,
--                                    subset_import_staging)
--   6. i18n                         (ui_strings)
--
-- Changes vs. V2 (2026-06-17):
--   - Gamification shipped: lumen_transactions, user_achievements,
--     user_momentum all now live (V2 listed these as "written in code,
--     migration not run"). user_streaks (daily-completion streak, separate
--     from lumens) is new on top of that, added after V2.
--   - New: webauthn_credentials (passkey sign-in), friend_invites ("Invite a
--     friend"), anne_messages (Anne mentor-chat history, FK to
--     learner_passports), learner_whois (single-point-of-truth learner
--     context for every LLM call — core_text + narrative_text, append-only
--     history, see server/services/whois.js), knowledge_estimate_log
--     (rollback log for the Knowledge Estimation add-on), lootbox_cache
--     (per-node cached Loot Box generation results).
--   - temp_export (V2's noted scratch table from a one-off Estonian
--     translation import) is gone — cleaned up since.
--   - users gained google_linked / discord_linked / linkedin_linked
--     (accurate "signed in with" reporting), password_reset_token /
--     password_reset_expires (forgot-password flow), avatar_url /
--     avatar_source (Google profile photo, ENUM('upload','google') so a
--     future upload feature can't be silently clobbered by the next Google
--     login).
--   - nodes gained an overview column (short LLM-generated blurb, populated
--     lazily/sparsely on demand — do not assume it's populated for a given
--     node).
--
-- Known drift vs. earlier design docs (kept here for the record, carried
-- forward from V2 — still true):
--   - knowledge_subsets.type is ENUM('personal','public','shared'), not
--     ('system','user') as originally designed; the table also picked up
--     icon_color, background_hidden, display_mode, is_overlay, ring_color,
--     source_url, version — none of which were in the original design.
--   - learner_passports never got pronouns/tagline columns that were once
--     planned; it has id_number instead.
--   - passport_reflections has an optional event_id FK back to
--     passport_events (a reflection can be attached to a specific event).
--
-- Design decisions (still true, verified against this snapshot):
...
--   - No true JSON-typed columns anywhere — candidates_json on
--     subset_import_staging (a deliberate exception for a throwaway staging
--     table) is stored as TEXT holding JSON-formatted content, not a native
--     JSON column type
...
--   - node labels are English by default; node_translations holds
--     per-locale overrides keyed by node external_id, COALESCEd at query time
--   - learner_whois is append-only (never UPDATE/DELETE an existing row) —
--     "the current entry" for a passport is just its newest row; older rows
--     are the retained history of how the learner's assessment evolved
-- ============================================================================
...
CREATE TABLE `edges` (
    ...
    ) ENGINE=InnoDB AUTO_INCREMENT=11084 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `friend_invites` (
                                  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `inviter_passport_id` bigint(20) unsigned NOT NULL,
    ...
    KEY `fk_invite_relationship` (`relationship_id`),
    CONSTRAINT `fk_invite_passport` FOREIGN KEY (`inviter_passport_id`) REFERENCES `learner_passports` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_invite_relationship` FOREIGN KEY (`relationship_id`) REFERENCES `passport_relationships` (`id`) ON DELETE CASCADE
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `knowledge_estimate_log`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `knowledge_estimate_log` (
                                          `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `passport_id` bigint(20) unsigned NOT NULL,
    `credential_id` bigint(20) unsigned DEFAULT NULL,
    `node_external_id` varchar(20) NOT NULL,
    `percentage` tinyint(3) unsigned NOT NULL,
    `retention_tier` enum('core','practiced','specialized') NOT NULL,
    `created_at` datetime NOT NULL DEFAULT current_timestamp(),
    PRIMARY KEY (`id`),
    KEY `idx_kel_passport` (`passport_id`),
    KEY `fk_kel_credential` (`credential_id`),
    CONSTRAINT `fk_kel_credential` FOREIGN KEY (`credential_id`) REFERENCES `passport_credentials` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_kel_passport` FOREIGN KEY (`passport_id`) REFERENCES `learner_passports` (`id`) ON DELETE CASCADE
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `learner_whois`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `learner_whois` (
                                 `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `passport_id` bigint(20) unsigned NOT NULL,
    ...
    CONSTRAINT `fk_whois_passport` FOREIGN KEY (`passport_id`) REFERENCES `learner_passports` (`id`) ON DELETE CASCADE
...
CREATE TABLE `nodes` (
    ...
    ) ENGINE=InnoDB AUTO_INCREMENT=11050 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `passport_credentials` (
    ...
    `sort_order` smallint(6) NOT NULL DEFAULT 0,
    `knowledge_estimated` tinyint(1) NOT NULL DEFAULT 0,
    PRIMARY KEY (`id`),
...
CREATE TABLE `user_achievements` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_passport_ach` (`passport_id`,`achievement_key`)
...
CREATE TABLE `user_momentum` (
    ...
    `streak_days` tinyint(3) unsigned NOT NULL DEFAULT 1,
    `multiplier` decimal(4,2) NOT NULL DEFAULT 1.00,
...
CREATE TABLE `users` (
    ...
    `role` enum('learner','admin','super_admin') NOT NULL DEFAULT 'learner',
    ...
    `discord_linked` tinyint(1) NOT NULL DEFAULT 0,
    `linkedin_linked` tinyint(1) NOT NULL DEFAULT 0,
    `password_reset_token` varchar(64) DEFAULT NULL,
    `password_reset_expires` datetime DEFAULT NULL,
    `avatar_url` varchar(500) DEFAULT NULL,
    `avatar_source` enum('upload','google') DEFAULT NULL,
    PRIMARY KEY (`id`),
