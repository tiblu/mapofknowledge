// Renders a full Learner Passport (as returned by _fetchFullPassport in
// server/routes/api.js) into plain prose for Anne's system prompt context.
// Distinct from llm.js's profileBlock() — that's a thin 6-field nudge used to
...
function renderPassportText(data) {
  ...
  if (goals.length) {
    const lines = goals.map(g => {
      ...
      const status = g.status === 'completed' ? 'completed' : `in progress, ${g.progress || 0}%`;
      const target = g.target_date ? `, target ${new Date(g.target_date).toISOString().slice(0, 10)}` : '';
