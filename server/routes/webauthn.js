...
// account that already exists via password, Google, or Discord. There is no
// passkey-only signup.
//
...
const RP_NAME = 'KnoBitz';
const RP_ID   = new URL(process.env.BASE_URL || 'https://knobitz.com').hostname;
const ORIGIN  = process.env.BASE_URL || 'https://knobitz.com';
...
router.delete('/credentials/:id', async (req, res) => {
  if (!_requireAuth(req, res)) return;
  try {
    // Same last-method protection as passkey deletion on themapofknowledge.com
    // — a passkey can be this account's only remaining way in just as easily
    // as an SSO provider can.
  ...
  try {
    ...
    if (isFirstLogin) {
      ...
      notify(user.id, 'welcome',
        locale === 'en' ? 'Welcome to KnoBitz!' : 'Tere tulemast KnoBitz-i!',
        locale === 'en'
    ...
    req.login(user, (err) => {
      ...
      const redirect = role === 'teacher' ? '/app/teacher.html'
                      : role === 'parent'  ? '/app/parent.html'
                      : '/app/';
